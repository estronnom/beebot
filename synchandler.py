...
def csvLoadSender(idUser, expense, callData):
    ...
    if not load:
        ...
        bot.send_document(idUser, csvCreator(headers, load), visible_file_name=f'{"Расходы" if expense else "Доходы"}.csv')
...
def insertDigit(message, direction, note, func):
    try:
        ...
        bot.send_message(message.chat.id, 'Число введено некорректо, попробуйте еще раз')
...
def photoHandler(message):
    ...
    if uploadPicture(file, message):
        ...
        bot.send_message(message.chat.id, 'С загрузкой фотографии произошла ошибка, обратитесь к администратору')
...
def messageHandler(message):
    getRole = db.ex('SELECT role FROM employee WHERE chatid = %s AND deleted IS NOT True',
                    (int(message.chat.id),))
    if not getRole:
        bot.send_message(message.chat.id, 'Такой пользователь не зарегистрирован в системе\nОбратитесь к администратору\n/authuser для авторизации')
    elif getRole[0][0] == 'owner':
        bot.send_message(message.chat.id, "Личный кабинет администратора", reply_markup=mk.createMarkup(2, ['Автомобили', 'Сотрудники', 'Затраты', 'Доходы', 'Сводка', 'Выдать З/П', 'Поездки', 'Зарплата', 'Подтвердить расходы'], ['adAuto', 'adEmployee', 'adExpenses', 'adIncome', 'adPivot', 'adWage', 'adObjects', 'adLoadWage', 'adApproveExpenses']))
    elif getRole[0][0] == 'user':
        bot.send_message(message.chat.id, 'Личный кабинет сотрудника', reply_markup=mk.createMarkup(1, ['Отчитаться о поездке', 'Мои доходы', 'Загрузить фотоотчет'], ['userTask', 'userIncome0', 'userUploadPicture']))
    else:
        bot.send_message(message.chat.id, 'Проблемы с ролью пользователя\nОбратитесь к администратору')
...
def auth(message):
    ...
    bot.send_message(message.chat.id, 'Запрос на авторизацию успешно отправлен, ожидайте ответа администратора')
...
def createTask(message):
    if not stack[message.chat.id]['taskObject']:
        ...
        carList = getAutoList()
        bot.send_message(message.chat.id, 'Теперь введите номер(цифру перед названием) автомобиля, на котором была совершена поездка\n' + carList)
    elif not stack[message.chat.id]['taskCar']:
        try:
            idCar = int(message.text)
            if not db.ex('SELECT * FROM auto WHERE id = %s', (idCar,)):
                bot.send_message(message.chat.id, 'Машины с таким id не найдено в базе, попробуйте еще раз')
            else:
                stack[message.chat.id]['taskCar'] = idCar
                employeeList = getEmployees(coeff=False)
                bot.send_message(message.chat.id, 'Отлично, теперь введите номер напарника, с которым вы выполняли задачу\nЕсли вы выполняли ее самостоятельно, то отправьте слово /skip\n\n' + employeeList)
        except ValueError:
            bot.send_message(message.chat.id, 'Число введено некорректно, попробуйте еще раз')
    elif not stack[message.chat.id]['taskBuddy']:
        if message.text == '/skip':
            stack[message.chat.id]['taskBuddy'] = 'flagSkipped'
            bot.send_message(message.chat.id, 'Супер, теперь введите количество километров, затраченных на поездку')
        else:
            try:
                idBuddy = int(message.text)
                if not db.ex('SELECT * FROM employee WHERE id = %s', (idBuddy,)):
                    bot.send_message(message.chat.id, 'Сотрудника с таким id не найдено в базе, попробуйте еще раз')
                else:
        ...
        insertDigit(message, 'taskKm', 'Отлично, теперь введите количество часов, которое заняла поездка', int)
    elif not stack[message.chat.id]['taskTime']:
        insertDigit(message, 'taskTime', 'Теперь введите сумму, полученную за заказ', int)
        ...
        if message.text == '/skip':
            ...
            bot.send_message(message.chat.id, 'Готово! Отчет успешно внесен в базу\n/office для перехода в личный кабинет')
            clearUserStack(message.chat.id)
        else:
            ...
            try:
                ...
                bot.send_message(message.chat.id, 'Сумма расхода введена некорректно\nВерный формат: 1000 на бензин')
            else:
        ...
        clearUserStack(message.chat.id)
        bot.send_message(message.chat.id, 'С отчетом о поездке произошла ошибка :(\nПерейдите в личный кабинет /office и попробуйте еще раз')
...
def addingAuto(message):
    if not stack[message.chat.id]['addingAutoName']:
        ...
        bot.send_message(message.chat.id, 'Теперь введите номерной знак машины')
    else:
        ...
        bot.send_message(message.chat.id, 'Готово! Машина добавлена в базу', reply_markup=mk.createMarkup(2, ['Добавить еще', 'Закончить'], ['adAutoAdd', 'endAutoAdd']))
...
def autoDeleter(message):
    ...
    bot.send_message(message.chat.id, f'Машина {message.text} удалена из базы', reply_markup=mk.createMarkup(2, ['Удалить еще', 'Закончить'], ['adAutoDelete', 'endAutoDelete']))
...
def addingEmployee(message):
    if not stack[message.chat.id]['updateEmployeeId']:
        insertDigit(message, 'updateEmployeeId', 'Теперь введите имя для сотрудника', int)
    elif not stack[message.chat.id]['updateEmployeeName']:
        stack[message.chat.id]['updateEmployeeName'] = message.text
        bot.send_message(message.chat.id, 'Теперь введите коэффициент зарплаты для сотрудника (введите 1, чтобы оставить сотрудника без коэффициента)')
    elif not stack[message.chat.id]['updateEmployeeCoeff']:
        insertDigit(message, 'updateEmployeeCoeff', 'Отлично, теперь введите часовую ставку сотрудника', float)
        ...
        bot.send_message(message.chat.id, 'Готово! Информация о сотруднике отредактирована', reply_markup=mk.createMarkup(2, ['Обновить еще', 'Закончить'], ['adEmployeeUpdate', 'endEmployeeUpdate']))
...
def deletingEmployee(message):
    ...
    bot.send_message(message.chat.id, f'Сотрудник {message.text} удален из базы', reply_markup=mk.createMarkup(2, ['Удалить еще', 'Закончить'], ['adEmployeeDelete', 'endEmployeeDelete']))
...
def callbackQuery(call):
    ...
    if call.data.startswith('auth'):
        ...
        bot.send_message(data[2], f'Ваши привилегии обновлены\nНовая роль: {rolemapping[data[3]]}\n/office для перехода в личный кабинет')
        bot.edit_message_text('Роль успешно обновлена', call.from_user.id, call.message.id, reply_markup=None)
        ...
        bot.send_message(call.from_user.id, 'Добавляем автомобиль в базу\nВведите название автомобиля')
    elif call.data == "endAutoAdd":
        stack[call.from_user.id]['addingAuto'] = False
        bot.send_message(call.from_user.id, 'Добавление закончено\n/office для входа в личный кабинет')
    elif call.data == "adAutoDelete":
        bot.send_message(call.from_user.id, 'Введите номер автомобиля, который желаете удалить из базы')
        stack[call.from_user.id]['deletingAuto'] = True
    elif call.data == "endAutoDelete":
        stack[call.from_user.id]['deletingAuto'] = False
        bot.send_message(call.from_user.id, 'Удаление автомобилей закончено\n/office для входа в личный кабинет')
        ...
        bot.send_message(call.from_user.id, 'Введите номер сотрудника, информацию о котором собираетесь редактировать')
        ...
        bot.send_message(call.from_user.id, 'Обновление закончено\n/office для входа в личный кабинет')
    elif call.data == "adEmployeeDelete":
        bot.send_message(call.from_user.id, 'Введите номер сотрудника, которого желаете удалить из базы')
        ...
        bot.send_message(call.from_user.id, 'Удаление сотрудников закончено\n/office для входа в личный кабинет')
    elif call.data == "adExpenses":
        bot.send_message(call.from_user.id, 'Выберите период выгрузки расходов', reply_markup=mk.createMarkup(2, ['24 часа', 'Неделя', 'Месяц', 'Все время'], ['loadExpenses1', 'loadExpenses7', 'loadExpenses30', 'loadExpensesAll']))
    elif call.data.startswith('loadExpenses'):
        csvLoadSender(call.from_user.id, True, call.data)
    elif call.data == "adIncome":
        # TODO: fix periods
        bot.send_message(call.from_user.id, 'Выберите период выгрузки доходов', reply_markup=mk.createMarkup(2, ['24 часа', 'Неделя', 'Месяц', 'Все время'], ['loadIncome1', 'loadIncome7', 'loadIncome30', 'loadIncomeAll']))
    elif call.data.startswith('loadIncome'):
        csvLoadSender(call.from_user.id, False, call.data)
    elif call.data == 'adPivot':
        bot.send_message(call.from_user.id, 'Выберите период выгрузки', reply_markup=mk.createMarkup(2, ['24 часа', 'Неделя', 'Месяц', 'Все время'], ['loadPivot1', 'loadPivot7', 'loadPivot30', 'loadPivotAll']))
        ...
        if load:
            ...
            bot.send_message(call.from_user.id, load, reply_markup=mk.createMarkup(1, ['Подтвердить выплату', 'Отклонить выплату'], [f'adWageStartApply//{employeeToTaskId}//{amount}', f'adWageStartReject//{employeeToTaskId}']))
        else:
            bot.send_message(call.from_user.id, 'Не найдено неоплаченных поездок\n/office')
        ...
        if not load:
            ...
            bot.send_document(call.from_user.id, csvCreator('ID;Дата;Объект;Сотрудник;Напарник;Авто;Километраж;Длительность;Доход;Расход\n', load), visible_file_name='Поездки.csv')
    elif call.data.startswith('adLoadWage'):
        if call.data == 'adLoadWage':
            bot.send_message(call.from_user.id, 'Выберите период выгрузки транзакций З\П', reply_markup=mk.createMarkup(2, ['24 часа', 'Неделя', 'Месяц', 'Все время'], ['adLoadWage1', 'adLoadWage7', 'adLoadWage30', 'adLoadWageAll']))
        else:
            ...
            if not load:
                bot.send_message(call.from_user.id, 'Выплат за данный период не найдено')
            else:
                bot.send_document(call.from_user.id, csvCreator('Сотрудник,Выплата\n', load), visible_file_name='Зарплаты.csv')
        ...
        if not load:
            bot.send_message(call.from_user.id, 'Неподтвержденных расходов не найдено\n/office для перехода в личный кабинет')
        else:
        ...
        bot.send_message(call.from_user.id, 'Отправьте фотографию для загрузки на диск администратора')
